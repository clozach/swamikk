...
2. Open Membership and receipts. The same invoice keeps its Paid status and original amount, with separate refund history immediately below. Refresh after another signed event and check that money changes without altering active/ended membership or content access.
3. Open a member purchase request and admin Refund review for that invoice. Check that a policy-pending request still waits for its decision even if an external refund succeeded. The amounts captured for its review stay distinct from newer observed money.
