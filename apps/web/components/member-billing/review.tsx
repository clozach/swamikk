...
import {
    ViewportDialog,
    type DialogFocusOrigin,
} from "@/components/ui/viewport-dialog";
...
export function CancellationReview({
    ...
    return (
        ...
        >
            <ViewportDialog kind="review" focusOrigin={focusOrigin}>
                ...
                <div className="kk-viewport-dialog-body" tabIndex={0}>
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-3 rounded-xl bg-muted/40 p-4 text-sm [&>dd]:text-right">
                        <dt>{copy.period}</dt>
                        <dd>
                            {date(quote.period.start)} –{" "}
                            {date(quote.period.end)}
                        </dd>
                        <dt>{copy.paid}</dt>
