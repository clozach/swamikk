...
For new class checkouts, the native checkout path should accept an explicitly selected cohort, validate it against the course and actual dated booking, and persist that association with the newly allocated invoice and membership session before leaving for Checkout. It should create a `source: checkout` evidence record from trusted server values, with provenance and the booked start. Do not populate this field from a current roster during an old receipt read, and do not silently backfill legacy invoice associations. This slice adds operator verification; it does not modify checkout.
