...
New native lessons begin with `publication.kind: never`; first publication atomically stores the database timestamp. Unpublishing/re-publishing does not reset it. Existing undated lessons and legacy membership starts are unknown, never assigned the observation/import date. Such content remains available during active membership but is omitted from retention and counted in `unknownReleaseCount`. Verified historical imports must provide actual evidence and dates; no automatic backfill guesses them.
