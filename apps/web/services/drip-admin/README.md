...
Eligible targets are native `course` and `download` products, including unpublished products being prepared. Blog articles are excluded from the picker, direct course-ID reads, new drafts and existing draft-ID actions. Empty sections left by a type conversion do not make a blog eligible. The same tenant and native course-management permission checks still apply; no article records or existing draft history are deleted by this filter.
...
Restoration restores the reviewed section order, effective release rules and notification setting, not byte-for-byte field absence. Returning to Available now sets `drip.status=false` and may retain the last dormant type/date and existing message template. Both the administrator rule projection and worker ignore that dormant date while status is false. A restoration report must say that the effective rules were restored rather than claiming the native document is identical to its preimage.
...
Both new and restoration drafts hold a shared `AccountLifecycle` write reservation through preparation and insertion. Beginning account closure rejects new reservations, including callers holding a stale administrator document. Direct user cleanup requires existing reservations to drain first, so a late draft cannot recreate private data after erasure. Crashed reservations do not expire automatically; account closure keeps them pending for explicit reconciliation. Approval/recovery continue to update existing records without upsert, and unsettled administrative operation locks remain available to another authorized administrator.
...
`app/api/drip-admin/__tests__/account-closure.test.ts` pauses each real draft insertion across account closure, checks that direct cleanup refuses a busy account, and confirms no new/restoration draft remains after erasure. It also proves a closing account cannot create another draft while its user row still exists.

`app/api/drip-admin/__tests__/target-eligibility.test.ts` reproduces article listing/detail and retained-empty-section draft eligibility against actual Mongo records, then verifies exclusion at list, course-ID and every draft-ID boundary. Native course/download approval, own-course permissions and tenant isolation remain covered. For the runtime check, open Releases and confirm the picker contains permitted courses/downloads only; an article's direct API ID must return unavailable without creating a release draft.
...
The approval table displays current and proposed release-message states alongside each section rule: on, off, or no prepared message. A notification-only change therefore remains visible even when dates and due-recipient counts are identical. These labels come from the saved review, not unsaved controls; changing controls still invalidates acknowledgment. Native rehearsal found and corrected the omitted transition. Verify by saving a notification-only draft, comparing both message states, then approving or discarding that reviewed version.
