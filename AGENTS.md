...
## Contextual comments and change review

Al retired the tester Journey Card in the September 6 PDF review. Do not mount it or add new journeys. Everyone uses the shared contextual comments control; public and member comments are text-only, while admins can attach photos and copy page feedback into a development prompt. Hold Ctrl/⌘ to select a component, or use the persistent ? button with keyboard and touch alternatives. Site changes require admin approval of a concrete version with visible consequences; failed attempts retain drafts and recovery. Keep the optional legacy builder behind its single Admin navigation seam. Verification steps and focused visual diffs belong in the owning project's Changes report, not a second site overlay.
