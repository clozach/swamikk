...
.${ROOT}__intro { margin: 0 auto 30px; max-width: 62ch; }
.${ROOT}__more { margin-top: 30px; text-align: center; }
.${ROOT}__more a { color: ${RUST}; text-decoration: underline; }
.${ROOT}__rows { display: flex; flex-direction: column; gap: 20px; }
@media (min-width: 768px) {
    .${ROOT}__rows .${ROOT}__card { flex-direction: row; max-width: 100%; gap: 30px; }
    .${ROOT}__rows .${ROOT}__media { width: 45%; flex-shrink: 0; }
}
...
export default function Widget({
    ...
    const shape = normalizeLayout(layout) ?? "grid";
    ...
    return (
        ...
        >
            ...
            <div className={ROOT}>
                ...
                {cards.length > 0 && (
                    <div
                        className={`${ROOT}__${shape === "row" ? "rows" : "grid"}`}
                    >
