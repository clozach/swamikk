...
import { ResponsiveImage, Link } from "@courselit/components-library";
...
function PostThumb({ post, index }: { post: Post; index: number }) {
    ...
    return (
        <div
            className="relative w-full aspect-[3/2] overflow-hidden md:rounded-t-[6px]"
            data-kk-image-path={
                source ? `posts.${index}.thumbnail.source` : undefined
            }
        >
            {isWaiting(source) ? (
...
function PostCard({ post, index }: { post: Post; index: number }) {
    return (
        ...
        >
            <PostThumb post={post} index={index} />
